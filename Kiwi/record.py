...
def record(station, port, freq, mode, time, schedule_id):
    ...
    command = ["python", "kiwirecorder.py", "-k", str(30), "--station", str(station) ,"-s", str(station), "-p", str(port), "-f", str(freq), "-m", mode,
               "--tlimit="+str(time), "-d", "audioFiles", "--fn", "schedule_{}".format(str(schedule_id))]
